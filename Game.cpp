...
{
    string title = "Snake Game Score: " + to_string(score) + " FPS: " + to_string(fps);
...
void Game::Render()
{
    ...
    if (alive) SDL_SetRenderDrawColor(renderer, 0x00, 0x7A, 0xCC, 0xFF);//blue color
    else       SDL_SetRenderDrawColor(renderer, 0xFF, 0x00, 0x00, 0xFF);//red color
    SDL_RenderFillRect(renderer, &block);
